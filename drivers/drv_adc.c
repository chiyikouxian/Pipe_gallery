...
/* ADC channel number is up to 18 */
...
#endif
    /* stop ADC first to ensure clean state for channel switching */
    HAL_ADC_Stop(stm32_adc_handler);
    
    /* Wait for ADC to stop completely */
    rt_uint32_t timeout = 100;
    while (HAL_ADC_GetState(stm32_adc_handler) != HAL_ADC_STATE_READY && timeout > 0)
    {
        timeout--;
        rt_thread_mdelay(1);
    }
    ...
    /* For STM32H7, completely disable ADC before reconfiguring channel */
    /* This ensures the channel configuration can be properly changed */
    ADC_Disable(stm32_adc_handler);
    ...
    /* Additional delay to ensure ADC is fully stopped */
    rt_thread_mdelay(3);
#else
    /* For other series, add a small delay to ensure ADC is fully stopped */
    rt_thread_mdelay(1);
#endif
    
    /* Configure the channel */
    /* This must be done after stopping the ADC to ensure proper channel switching */
    HAL_StatusTypeDef hal_status = HAL_ADC_ConfigChannel(stm32_adc_handler, &ADC_ChanConf);
    if (hal_status != HAL_OK)
    {
        LOG_E("Failed to configure ADC channel %d, status: %d", channel, hal_status);
        return -RT_ERROR;
    }

#if defined(SOC_SERIES_STM32H7)
    /* Re-enable ADC after channel configuration */
    ADC_Enable(stm32_adc_handler);
    ...
    *value = (rt_uint32_t)HAL_ADC_GetValue(stm32_adc_handler);
    ...
    /* stop ADC after reading to allow channel switching */
    HAL_ADC_Stop(stm32_adc_handler);
    
    /* Wait for ADC to stop before next channel switch */
    timeout = 100;
    while (HAL_ADC_GetState(stm32_adc_handler) != HAL_ADC_STATE_READY && timeout > 0)
    {
        timeout--;
...
static int stm32_adc_init(void)
{
    ...
    for (i = 0; i < sizeof(adc_config) / sizeof(adc_config[0]); i++)
    {
        ...
        {
#if defined(SOC_SERIES_STM32H7)
            /* ADC calibration for STM32H7 (improves accuracy) */
            if (HAL_ADCEx_Calibration_Start(&stm32_adc_obj[i].ADC_Handler, ADC_CALIB_OFFSET, 0) != HAL_OK)
            {
                LOG_W("%s calibration failed", name_buf);
                /* Continue even if calibration fails */
            }
            else
            {
                LOG_D("%s calibration success", name_buf);
            }
#endif
            /* register ADC device */
