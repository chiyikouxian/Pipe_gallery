...
/* debug option - disabled for MSH input */
//#define ETH_RX_DUMP
//#define ETH_TX_DUMP
//#define DRV_DEBUG
...
{
    /* LAN8720A module has no nRST pin — software reset is performed
     * via MDIO (BCR bit 15) later in rt_stm32_eth_init() */
}
...
static rt_err_t rt_stm32_eth_init(rt_device_t dev)
{
    ...
    {
        LOG_W("HAL_ETH_Init failed, retrying...");
        rt_thread_mdelay(500);
        if (HAL_ETH_Init(&EthHandle) != HAL_OK)
        {
        ...
        {
            LOG_D("eth hardware init success (retry)");
        }
    }
    else
    {
